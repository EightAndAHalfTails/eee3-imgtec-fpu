...
int main(){
	...
	while(1){
		...
		switch(opcode){
		case 0: cout<<"Null operation"; break;
...
float adder(float a_f, float b_f, bool IsSub){
	...
	/*
	//normalise
	if(z.m & (0xFFFFFFFF << (24+d)) != 0){		
		z.e += 1;
		d += 1;
	}
	
	//rounding
	if (((z.m >> (d-1)) & 0x00000001) && ((z.m & (0xFFFFFFFF >> (31-d)) != 0))){	//condition is true if round up necessary
		z.m >>= d;
		z.m += 1;
	}
	else{
