...
void round_norm(fp_t &z, int, bool, bool);
...
int main(){
	...
	while(1){
		...
		case 15: cout<<"No op";
			...
			inname += no;
			inname += ".txt";
			testMultiplication(inname, "outputing.txt");
			break;
...
void round_norm(fp_t &z, int lostbits, bool IsUp, bool IsMult){
	...
	if(IsMult | (firstexp != 0 & z.e == 0)){
		lostbits <<= 1;
		LSB = extractBits(0,0,z.m);
		lostbits += LSB;
		z.m >>= 1;
	}
	//cout<<dec<<z.e<<hex<<"-"<<z.m<<endl;
...
fp_t multiplier(fp_t x, fp_t y){
	...
	// New exponent
	z.e = x.e + y.e - 127;
	if(z.e != 0 | (z.e == 0 & x.e != 0)){
		z.e++;	// due to Booth encoding, we do x*2y and reduce later
	}			// denormals do not need to de reduced though
	...
	// compensates for Booth multiplication of x*2y, by halving result
	lostbits <<= 1;
	LSB = extractBits(0,0,z.m);
	lostbits += LSB;
	z.m >>= 1;
	
	// if prodcut is now denormal, we do not need to compensate
	if(x.e+y.e <= 126 & x.e != 0){
	//if( (x.e+y.e <= 126 & x.e != 0) | (x.e == 0 & y.e == 128) ){
		//z.e--;
		IsMult = 1;
	}
	
	// this takes account of the leading 1 in normal significands
	lostbits <<= 1;
	LSB = extractBits(0,0,z.m);
	lostbits += LSB;
	z.m >>= 1;
	...
	round_norm(z, lostbits, 1, IsMult);
	//cout<<"z: "<<dec<<z.s<<"-"<<z.e<<"-"<<hex<<z.m<<endl<<endl;
